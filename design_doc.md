...
| :--- | :--- | :--- |
| **Direct & Paraphrased Prompt Injections** (*"Ignore instructions"*, *"disregard guidance given earlier"*) | **Two-Layer Security Scanning**: Layer 1 fast-pass heuristic keyword check + Layer 2 semantic LLM classification (`InjectionScanResult`). Wrapped in XML tags with NFKD Unicode homoglyph normalization. | Flag: `possible_prompt_injection`<br>Decision: `Needs Manual Review` |
| **Failed / Malformed JD Extraction** (*Model API error, rate limit, or invalid JSON response*) | **Loud Exception Safeguard (`JDExtractionError`)**: `extract_jd_requirements()` raises `JDExtractionError` and halts pipeline execution before evaluating candidate resumes, eliminating silent fallback evaluations. | Pipeline Halted<br>Error: `JDExtractionError` |
